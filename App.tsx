...
import { generatePetPortrait, validateImageContent } from './services/geminiService';
import { UploadIcon, TrashIcon, SparklesIcon, DownloadIcon, WarningIcon, ArrowLeftIcon, ArrowRightIcon } from './components/icons';
...
const App: React.FC = () => {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>, fileType: 'pet' | 'owner') => {
    ...
    // 1. Buat objek placeholder dan tambahkan ke state segera untuk umpan balik UI.
    ...
    // 2. Mulai validasi untuk setiap file baru dan perbarui state-nya setelah selesai.
    newFilePlaceholders.forEach(placeholder => {
      validateImageContent(placeholder.file, fileType)
        .then(isValid => {
          const warning = !isValid 
            ? (fileType === 'pet'
                ? 'Peringatan: Gambar ini mungkin tidak berisi hewan peliharaan.'
                : 'Peringatan: Gambar ini mungkin tidak berisi orang.')
            : undefined;
        ...
        .catch(err => {
          ...
          const warning = "Tidak dapat memvalidasi konten gambar.";
          setFiles(prevFiles => 
            prevFiles.map(f => 
              f.id === placeholder.id 
                ? { ...f, validating: false, warning } 
  ...
  const constructedPrompt = useMemo(() => {
    ...
    if (currentImage && revisionPrompt) {
      ...
      promptParts.push(`[ATURAN PENTING]: Jangan mengubah apa pun pada gambar dasar yang tidak secara eksplisit diminta dalam instruksi revisi. Pertahankan gaya, komposisi, pencahayaan, dan identitas subjek asli sedekat mungkin.`);
      promptParts.push(`[FORMAT OUTPUT]:\nRasio Aspek: ${aspectRatio}`);
      return promptParts.join('\n\n');
    }

    // ---> LOGIKA PEMBUATAN AWAL <---
    ...
    switch (photoConcept) {
        ...
        case 'themed_set':
            if (activeTheme === 'Tema Liburan') {
                basePrompt = `Ciptakan foto hiperrealistis yang menampilkan subjek sedang menikmati liburan. Tempatkan mereka di salah satu lokasi liburan yang khas, seperti pantai yang cerah dengan air biru jernih, terminal bandara yang ramai, taman kota yang indah, atau taman bermain yang menyenangkan. Suasananya harus santai dan ceria, menangkap esensi dari sebuah liburan. Subjek (manusia dan hewan) harus mengenakan pakaian dan/atau aksesori yang sesuai dan realistis untuk suasana liburan, BUKAN diubah menjadi karakter non-realistis. Fokus pada detail, pencahayaan alami, dan material yang realistis untuk membuat adegan terasa seperti foto liburan nyata.`;
            } else {
    ...
    if (petFiles.length > 0 || ownerFiles.length > 0) {
        promptParts.push(`[SUBJEK]:\nKarakter utama untuk adegan ini disediakan dalam gambar yang diunggah. Ini adalah *satu-satunya* subjek yang harus Anda gunakan. Pastikan SEMUA subjek yang diunggah (manusia dan hewan) muncul bersama dalam satu bingkai gambar yang sama.`);
    }

    if (customPrompt) {
        promptParts.push(`[INSTRUKSI KUSTOM]:\n${customPrompt}`);
    }
...
2. **PERTAHANKAN IDENTITAS & POSE NATURAL (ATURAN PALING KRUSIAL):**
   - **Untuk Manusia:** Wajah dan proporsi tubuh (bentuk/berat badan) subjek manusia HARUS dipertahankan dengan kemiripan 100% dari foto yang diunggah. Ini adalah prioritas utama; jangan mengubah fitur wajah atau identitas mereka. Namun, Anda memiliki kebebasan kreatif penuh untuk mengubah **pakaian, properti, gestur, dan pose tubuh** mereka agar sesuai secara natural dengan tema dan konsep adegan. Ciptakan pose baru yang dinamis dan cocok, jangan hanya menyalin pose dari foto asli.
   - **Untuk Hewan:** Pertahankan sepenuhnya kemiripan fisik hewan. Pose hewan harus selalu **natural dan sesuai dengan spesiesnya**, bukan postur seperti manusia.
3. **KUALITAS OUTPUT TINGGI.** Gambar akhir harus berkualitas tinggi, dengan pencahayaan yang baik, dan komposisi artistik.`;
    ...
    return promptParts.join('\n\n');
  }, [selectedTheme, customTheme, aspectRatio, petFiles.length, ownerFiles.length, customPrompt, revisionPrompt, currentImage, photoConcept, imaginativeTheme]);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      // Jika ini adalah revisi, ubah gambar saat ini menjadi File dan tambahkan ke depan.
      if (currentImage && revisionPrompt) {
        ...
        // Gambar yang akan direvisi adalah konteks utama, jadi letakkan di awal.
      ...
      setRevisionPrompt(''); // Hapus prompt revisi setelah berhasil digunakan
    } catch (e) {
  ...
  const handleDownload = () => {
    ...
    link.download = 'genno-ai-mahakarya.png';
    ...
    <div className="flex flex-col lg:flex-row min-h-screen bg-gray-900 text-gray-200 font-sans">
      ...
      <aside className="w-full lg:w-[450px] bg-gray-900/80 backdrop-blur-sm border-r border-gray-700/50 p-6 flex flex-col space-y-6 overflow-y-auto">
        ...
        <section>
          ...
          <div>
            <label htmlFor="custom-prompt" className="block text-sm font-medium text-gray-300 mb-1">Instruksi Kustom (untuk pembuatan awal)</label>
